...
export default {
  ...
  async execute(client) {
          ...
          await import(fileURL)
            .then((command) => {
              if ("data" in command && "execute" in command) {
                ...
                lg.success(
                  `Команда ${command.data.name} завантажена з ${fullPath}`,
                );
              } else {
            ...
            .catch((error) =>
              lg.error(`Помилка завантаження команди ${fullPath}:`, error),
            );
        }
      }
    }
    ...
    const globalCommands = client.commands.map((command) =>
      command.data.toJSON(),
    );
    const devCommands = client.devCommands.map((command) =>
      command.data.toJSON(),
    );
    ...
    try {
      lg.info(
        `Реєстрація глобальних команд: ${globalCommands.map((c) => c.name).join(", ")}`,
      );
    ...
    try {
      lg.info(
        `Реєстрація dev-команд для ${guildId}: ${devCommands.map((c) => c.name).join(", ")}`,
      );
