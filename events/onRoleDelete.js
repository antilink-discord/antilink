...
export default {
    ...
    once: false,
    async execute(role) {
        console.log(`🔥 Роль видалена: ${role.name}`);
        await handleRoleAction(role, 'RoleDelete', DELETE_LIMIT, 'Антикраш: масове видалення ролей');
    }
};

// 🔹 **Обробка дії (видалення ролі)**
async function handleRoleAction(role, type, limit, reason) {
    setImmediate(async () => {
        try {
            ...
            const fetchedLogs = await guild.fetchAuditLogs({ type: AuditLogEvent.RoleDelete, limit: 1 }).catch(() => null);
