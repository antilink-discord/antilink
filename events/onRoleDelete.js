...
import { 
    add_role_delete_to_cache, 
    check_role_delete_cache
...
export default {
    ...
    once: false,
    async execute(role) {
        lg.warn('Бачу видалення ролі')
        setImmediate(async () => {
            try {
                const guildId = role.guild.id;
                lg.debug('Айді серверу: ', guildId)
                ...
                await Promise.all([ 
                    ...
                    add_role_delete_to_cache(guildId, executor.id)
                ]);

                const deleteCount = await check_role_delete_cache(guildId, executor.id);
